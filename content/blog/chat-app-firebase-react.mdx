...
![ezgif.com-gif-maker.gif](How%20to%20build%20a%20chatroom%20app%20with%20Firebase%20and%20Reac%20563d22bc1bab437bbb0b1538105abf9b/ezgif.com-gif-maker.gif)

At the end of this tutorial, I’ll give you some ways to extend this application to further your React and Firebase skills.
...
## What are Firebase Cloud Firestore and Firebase Authentication?
...
A Firebase project is a container for Firebase apps and its resources and services, such as Firestore databases and Authentication providers. A Firebase app (e.g., the web app or iOS app) belongs to a project; a project can have many apps, and all of its apps share the same resources and services.
...
Now that the project is created, let’s create the Firebase app. Since we’re adding Firebase to a React app, we’ll need to create a web app.
...
![Screen Shot 2022-06-08 at 8.31.53 PM.png](How%20to%20build%20a%20chatroom%20app%20with%20Firebase%20and%20Reac%20563d22bc1bab437bbb0b1538105abf9b/Screen_Shot_2022-06-08_at_8.31.53_PM.png)

Keep this page open; we’ll come back to it in the next section to grab your Firebase configuration.
...
In this section, we’ll add Firebase Authentication to our app, create a function to log in as a user with Google, and set up the context that we briefly discussed in the previous section.

We’ll create an `<AuthProvider>` component that passes down a `user` object and a `login` function. `login` wraps the SDK’s Google sign-in function and then sets the authenticated user in the state.
...
Finally, run `yarn start` and navigate to `http://localhost:3000`. Since `user` is initialized as `null` in our `<AuthProvider>`, you should see the text “I’m unauthenticated!”:

![Screen Shot 2022-06-08 at 8.44.25 PM.png](How%20to%20build%20a%20chatroom%20app%20with%20Firebase%20and%20Reac%20563d22bc1bab437bbb0b1538105abf9b/Screen_Shot_2022-06-08_at_8.44.25_PM.png)
...
![Screen Shot 2022-06-08 at 8.45.38 PM.png](How%20to%20build%20a%20chatroom%20app%20with%20Firebase%20and%20Reac%20563d22bc1bab437bbb0b1538105abf9b/Screen_Shot_2022-06-08_at_8.45.38_PM.png)
...
Having the ability to chat with others is great, but it’d be more fun to chat with people about different topics. We’ll allow this by creating hardcoded chat room topics. In this section, we’ll create hardcoded chat rooms and set up routing so that we can have different routes for each room (`/room/{roomId}`).
...
Let’s first create files for our two new components and put placeholder components in them.
...
`<Landing>` will be responsible for listing all of our chatrooms; clicking one of them will navigate to `/room/:id`. Add a placeholder component in `src/components/Landing/index.jsx`:
...
Now, let’s set up the router in `<AuthenticatedApp>` and render the routes with our new components. Replace our placeholder implementation in `src/components/AuthenticatedApp/index.jsx` with the following:
...
Discussing navigation with React Router is somewhat out of the scope of this article; if you’re interested in learning more about React Router, check out their documentation [here](https://reactrouter.com/docs/en/v6).
...
![Screen Shot 2022-06-08 at 8.52.52 PM.png](How%20to%20build%20a%20chatroom%20app%20with%20Firebase%20and%20Reac%20563d22bc1bab437bbb0b1538105abf9b/Screen_Shot_2022-06-08_at_8.52.52_PM.png)
...
![Screen Shot 2022-06-08 at 8.54.37 PM.png](How%20to%20build%20a%20chatroom%20app%20with%20Firebase%20and%20Reac%20563d22bc1bab437bbb0b1538105abf9b/Screen_Shot_2022-06-08_at_8.54.37_PM.png)
...
![Screen Shot 2022-06-08 at 8.59.53 PM.png](How%20to%20build%20a%20chatroom%20app%20with%20Firebase%20and%20Reac%20563d22bc1bab437bbb0b1538105abf9b/Screen_Shot_2022-06-08_at_8.59.53_PM.png)

Once we add data, the data viewer will display the structure of our data and allow us to view, add, edit, and delete them.

Recall that Firestore data are stored in key-value documents, which are grouped into collections. Every document must belong to a collection. Documents are JSON-like; for example, a document for a `dogs` chatroom could be structured like this:
...
Our `sendMessage` function takes in the `roomId`, current `user` (the object stored in context that we obtain using Authentication), and the message `text`. We use these data to form the document object passed as the second argument to `addDoc`.
...
![Screen Shot 2022-06-08 at 9.02.48 PM.png](How%20to%20build%20a%20chatroom%20app%20with%20Firebase%20and%20Reac%20563d22bc1bab437bbb0b1538105abf9b/Screen_Shot_2022-06-08_at_9.02.48_PM.png)
...
![Screen Shot 2022-06-08 at 9.04.25 PM.png](How%20to%20build%20a%20chatroom%20app%20with%20Firebase%20and%20Reac%20563d22bc1bab437bbb0b1538105abf9b/Screen_Shot_2022-06-08_at_9.04.25_PM.png)
...
We pass it a query which we construct using the `query` function. In our case, we want to listen to changes to a room’s `messages` subcollection and order the documents in ascending order by their `timestamp`.

The second argument we give it is a callback which gets called when it receives the initial query and any subsequent updates (i.e., when new documents are added). We form an array of messages by `map`ping each document, and then call the `callback` with the formatted messages. When we call `getMessages` in our hook, we’ll pass a `callback` so that we can store the messages in state.
...
And add styles to `src/components/MessageList/styles.css`:
...
Congrats—you now have a working chatroom app built with React and Firebase!
