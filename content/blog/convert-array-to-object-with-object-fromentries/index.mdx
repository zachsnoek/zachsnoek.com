...
This means that the `.reduce` method with the object spread has a time complexity of O(n^2), while `Object.fromEntries` is O(1). You can compare the performance of these two object-building methods in the chart below 😬:
